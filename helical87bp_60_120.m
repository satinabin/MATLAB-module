...
% 将线段转为垂直于x-y平面
for k = 1:N
    rotate(lnHandle{k}, cross([0 0 1], [-6.225 10.78 3.594]), -acos(dot([0 0 1], [-6.225 10.78 3.594])/(norm([-6.225 10.78 3.594])))/pi*180, [0 0 0]);
end

% %将线段平移
for k=1:N
    lnHandle{k}.XData = lnHandle{k}.XData-9.802;
    lnHandle{k}.YData = lnHandle{k}.YData-5.64;
    lnHandle{k}.ZData = lnHandle{k}.ZData+6.788;
end
