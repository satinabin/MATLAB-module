...
d = 6.32; %线径,得下面手动修改
dtheta = 90; %每一段对应的角度（此处修改为60）
turning = 300; %每一段的旋转角度
N = 50; %段数
...
% 将线段转为垂直于x-y平面
for k = 1:N
    rotate(lnHandle{k}, cross([0 0 1], [-3.597 3.597 -6.229]), -acos(dot([0 0 1], [-3.597 3.597 -6.229])/(norm([-3.597 3.597 -6.229])))/pi*180, [0 0 0]);
end

% %将线段平移
for k=1:N
    lnHandle{k}.XData = lnHandle{k}.XData-2.827;
    lnHandle{k}.YData = lnHandle{k}.YData-2.831;
    lnHandle{k}.ZData = lnHandle{k}.ZData+6.321;
end
