...
% 将线段转为垂直于x-y平面
for k = 1:N
    rotate(lnHandle{k}, cross([0 0 1], [-3.113 1.797 -5.391]), -acos(dot([0 0 1], [-3.113 1.797 -5.391])/(norm([-3.113 1.797 -5.391])))/pi*180, [0 0 0]);
end

% %将线段平移
for k=1:N
    lnHandle{k}.XData = lnHandle{k}.XData-1.093;
    lnHandle{k}.YData = lnHandle{k}.YData-1.89;
    lnHandle{k}.ZData = lnHandle{k}.ZData+6.795;
end