...
% 将线段转为垂直于x-y平面
for k = 1:N
    rotate(lnHandle{k}, cross([0 0 1], [-7.707 7.707 4.45]), -acos(dot([0 0 1], [-7.707 7.707 4.45])/(norm([-7.707 7.707 4.45])))/pi*180, [0 0 0]);
end

% %将线段平移
for k=1:N
    lnHandle{k}.XData = lnHandle{k}.XData-6.075;
    lnHandle{k}.YData = lnHandle{k}.YData-6.078;
    lnHandle{k}.ZData = lnHandle{k}.ZData+9.256;
end
