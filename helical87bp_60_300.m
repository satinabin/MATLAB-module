...
% 将线段转为垂直于x-y平面
for k = 1:N
    rotate(lnHandle{k}, cross([0 0 1], [-3.853 6.674 -6.674]), -acos(dot([0 0 1], [-3.853 6.674 -6.674])/(norm([-3.853 6.674 -6.674])))/pi*180, [0 0 0]);
end

% %将线段平移
for k=1:N
    lnHandle{k}.XData = lnHandle{k}.XData-6.054;
    lnHandle{k}.YData = lnHandle{k}.YData-3.495;
    lnHandle{k}.ZData = lnHandle{k}.ZData+5.34;
end
